...
data <- data.frame(date = d, Open = ss$Open, Close = ss$Close, invest = ifelse(ddd>0,1000,0), return = dailyReturn(ss))
...
```{r}
tt <- rep(0, nrow(data))
tt[1] <- data$invest[1] * (1 + data$daily.returns[1])
for (i in 2:nrow(data)) {
    tt[i] <- (tt[i - 1] + data$invest[i]) * (1 + data$daily.returns[i])
}
data1 <- data
data1$total <- tt
head(data1, 100)
